...
export const CheckoutModal = ({
  ...
  const totalPhp = hasOrderBump ? basePhp + bumpPhp : basePhp;
          ...
          <div className="p-8 text-center space-y-5">
            ...
            <div className="bg-slate-50 border border-slate-200 rounded-2xl p-4 max-w-md mx-auto text-left space-y-2 text-xs text-slate-700">
              ...
              <div className="flex justify-between">
                ...
                <span className="font-bold text-emerald-600 text-sm">₱{totalPhp.toLocaleString()} PHP</span>
              </div>
              <div className="flex justify-between">
          ...
          <form onSubmit={handleCheckoutSubmit} className="p-6 sm:p-7 space-y-6">
            ...
            <div className="bg-slate-50 rounded-2xl p-4 border border-slate-200 flex flex-col sm:flex-row items-center justify-between gap-4">
              <div>
                ...
                <span className="text-[11px] text-slate-400 font-medium">Approx. \${baseUsd} USD equivalent</span>
              </div>
            ...
            <div className="space-y-3">
              ...
              {paymentMethod === 'gcash' && (
                <div className="p-4 rounded-2xl bg-blue-50/70 border border-blue-200 space-y-3 animate-in fade-in duration-150">
                  ...
                  <div className="p-3 rounded-xl bg-white border border-blue-100 flex items-center gap-3 text-xs text-slate-600">
                    <QrCode className="w-8 h-8 text-[#007DFE] shrink-0" />
                    <div>
                      <span className="font-bold text-slate-900 block">Instant QRPh GCash Scan Available</span>
                      <span className="text-[11px] text-slate-500">Upon clicking Enroll, your invoice will be confirmed with instant access.</span>
                    </div>
                  </div>
                </div>
              )}
              ...
              {paymentMethod === 'gotyme' && (
                <div className="p-4 rounded-2xl bg-cyan-50/70 border border-cyan-200 space-y-3 animate-in fade-in duration-150">
                  ...
                  <div>
                    <label className="block text-[11px] font-semibold text-slate-700 mb-1">
                      GoTyme Reference Number or Sender Account (Optional for Instant Sync)
                    </label>
                    <input
                      type="text"
                      value={gotymeRef}
                      onChange={(e) => setGotymeRef(e.target.value)}
                      placeholder="e.g. GT-83920194 or 09XXXXXXXXX"
                      className="w-full px-3.5 py-2 rounded-xl border border-cyan-200 bg-white text-sm focus:outline-none focus:ring-2 focus:ring-[#00B4D8]"
                    />
                  </div>
                </div>
              )}
            ...
            <div className="pt-2">
              <button
                ...
                className="w-full py-4 px-6 rounded-2xl bg-gradient-to-r from-blue-600 to-indigo-600 hover:from-blue-700 hover:to-indigo-700 text-white font-black text-base shadow-xl shadow-blue-500/25 flex items-center justify-center gap-2 transition-all cursor-pointer disabled:opacity-75"
              >
                {isProcessing ? (
                  <span>Connecting to {paymentMethod.toUpperCase()} Gateway...</span>
                ) : (
                  <>
